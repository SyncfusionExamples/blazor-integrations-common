...
description: Improved step-by-step guide for building a shopping cart in a Blazor application, covering product listing, cart management, and checkout.
platform: Blazor
control: Common
documentation: ug
---
...
This section explains how to create a shopping cart workflow in a Blazor application. The walkthrough covers the essential building blocks required for a typical e‑commerce scenario, including defining data models, managing cart state using dependency‑injected services, and implementing pages for product listing, shopping cart management, and checkout.

The examples in this guide use Syncfusion Blazor components to build a clean, responsive, and enterprise‑ready user interface. 
...
* [Syncfusion.Blazor.Notifications](https://www.nuget.org/packages/Syncfusion.Blazor.Notifications)
...
{% tabs %}
{% highlight cs tabtitle="~/Program.cs" hl_lines="1 6" %}
...
### Create the CartItems component

This component renders an individual cart item and provides controls to update quantity or remove the item from the cart.

{% tabs %}
{% highlight razor tabtitle="Components/CartItems.razor"  %}
    ...
    <div class="row g-0">
        <div class="col-md-2">
            <img src="@Item.ImageUrl" class="img-fluid rounded-start" 
                 alt="@Item.ProductName" style="height: 150px; object-fit: cover;" />
        </div>
        <div class="col-md-6">
            <div class="card-body">
                <h5 class="card-title">@Item.ProductName</h5>
                <p class="card-text">
                    <span class="text-muted">@Item.UnitPrice.ToString("C")</span> each
                </p>
                <div class="d-flex align-items-center gap-2">
                    <SfButton Content="−" Size="Small" CssClass="e-outline"
                             OnClick="() => DecreaseQty()" />
        ...
        <div class="col-md-4 text-end d-flex flex-column justify-content-center pe-3">
            <h5 class="text-primary">@Item.Subtotal.ToString("C")</h5>
            <SfButton Content="Remove" CssClass="e-outline e-danger mt-2"
...
@code {
    [Parameter] public Models.CartItem Item { get; set; } = null!;
    [Inject] ICartService CartService { get; set; } = null!;

    private void IncreaseQty() => CartService.UpdateQuantity(Item.ProductId, Item.Quantity + 1);
    private void DecreaseQty() => CartService.UpdateQuantity(Item.ProductId, Item.Quantity - 1);
...
This component receives a cart item as a parameter and displays its details, quantity, and subtotal. User actions directly update the shared cart state through the cart service, keeping the cart page and cart badge synchronized.
...
<style>
    /* Hide outer page scrollbar while viewing catalog — only the catalog scrolls */
    html, body {
        overflow-y: hidden;
    }
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                <div class="col-12">
                                    ...
                                    <input type="text" class="form-control" @bind="order.Shipping.FullName" required />
                                </div>
                                <div class="col-12">
                                    <label class="form-label">Address *</label>
                                    <input type="text" class="form-control" @bind="order.Shipping.Address" required />
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">City *</label>
                                    <input type="text" class="form-control" @bind="order.Shipping.City" required />
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">State *</label>
                                    <input type="text" class="form-control" @bind="order.Shipping.State" required />
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">ZIP Code *</label>
                                    <input type="text" class="form-control" @bind="order.Shipping.ZipCode" required />
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Country *</label>
                                    <input type="text" class="form-control" @bind="order.Shipping.Country" required />
                                </div>
                                <div class="col-12">
                                    <label class="form-label">Phone *</label>
                                    <input type="tel" class="form-control" @bind="order.Shipping.Phone" required />
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="row g-3">
                                <div class="col-12">
                                    ...
                                    <input type="text" class="form-control" 
                                           ...
                                           @bind="order.Payment.CardNumber" required />
                                </div>
                                <div class="col-12">
                                    <label class="form-label">Card Holder Name *</label>
                                    <input type="text" class="form-control" 
                                           placeholder="JOHN DOE"
                                           @bind="order.Payment.CardHolder" required />
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Expiry Date (MM/YY) *</label>
                                    <input type="text" class="form-control" 
                                           placeholder="12/25"
                                           @bind="order.Payment.ExpiryDate" required />
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">CVV *</label>
                                    <input type="text" class="form-control" 
                                           placeholder="123"
                                           @bind="order.Payment.CVV" required />
                                </div>
                            </div>
                        </div>
                    </div>
...
N> This sample collects payment details for demonstration only.
                    ...
                    <div class="card p-3">
                        <div class="d-flex justify-content-between align-items-start">
                            ...
                            <div class="text-end">
                                ...
                                <a href="/order-confirmation/@o.OrderId" class="btn btn-outline-primary btn-sm mt-2">View</a>
                            </div>
                        </div>
                        <hr />
